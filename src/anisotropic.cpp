...
public:
	Anisotropic(const PropertyList &propList) {
		...
		m_alphaX = propList.getFloat("alphaX", 0.1f);
		m_alphaY = propList.getFloat("alphaY", 0.1f);
	...
	float microfacet_D(Vector3f wh, float alpha) const {
		float D = expf(-Frame::tanTheta(wh) * Frame::tanTheta(wh) / (m_alphaX * m_alphaY)) /
			(M_PI * m_alphaX * m_alphaY * powf(Frame::cosTheta(wh), 4));
		return D;
	}

	float microfacet_G(const BSDFQueryRecord &bRec, Vector3f wh, Normal3f n, float alpha) const {
		alpha = sqrt(Frame::cosPhi(wh)*Frame::cosPhi(wh) * m_alphaX * m_alphaY + Frame::sinPhi(wh)*Frame::sinPhi(wh) * m_alphaX * m_alphaY);
	...
	virtual Color3f eval(const BSDFQueryRecord &bRec) const {
		...
		float D = microfacet_D(wh, m_alpha);
		float G = microfacet_G(bRec, wh, n, m_alpha);
		float F = fresnel(wh.dot(bRec.wi), m_extIOR, m_intIOR);
		float cosThetaIO = 4 * Frame::cosTheta(bRec.wi) * Frame::cosTheta(bRec.wo);


		Color3f result = m_kd * INV_PI + m_ks * D * F * G / cosThetaIO;
		return result;
	}
	...
	virtual float pdf(const BSDFQueryRecord &bRec) const {
		...
		Normal3f n = Normal3f(0, 0, 1);
		Vector3f wh = (bRec.wi + bRec.wo) / (bRec.wi + bRec.wo).norm();
		float wPdf = Warp::squareToBeckmannPdf2(wh, m_alphaX, m_alphaY);
	...
	virtual Color3f sample(BSDFQueryRecord &bRec, const Point2f &_sample) const {
		...
		if (sample.x() < m_ks) {
			...
			sample.x() /= m_ks;
			Vector3f wh = Warp::squareToBeckmann2(sample, m_alphaX, m_alphaY);
