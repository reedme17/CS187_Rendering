...
public:
    ...
    static float mapParameter(WarpType warpType, float parameterValue) {
        if (warpType == CosinePowerHemisphere || warpType == PhongBRDF || warpType == BlinnBRDF)
            parameterValue = std::exp(std::log(0.0125f) * (1 - parameterValue) +
                                      std::log(500.f)   *  parameterValue);
        else if (warpType == Beckmann || warpType == MicrofacetBRDF)
    ...
    std::pair<Point3f, float> warpPoint(WarpType warpType, const Point2f &sample, float parameterValue) {
        ...
        switch (warpType) {
            ...
            case Triangle: result << Warp::squareToUniformTriangle(sample), 0; break;
            ...
            case CosinePowerHemisphere: result << Warp::squareToCosinePowerHemisphere(sample, parameterValue); break;
    ...
    void refresh() {
        ...
        else if (warpType == PhongBRDF || warpType == BlinnBRDF) {
            PropertyList list;
            list.setFloat("exponent", parameterValue);
            list.setColor("kd", Color3f(0.f));

            m_brdf = std::unique_ptr<BSDF>((BSDF *) NoriObjectFactory::createInstance(warpType == PhongBRDF ? "phong" : "blinn", list));
        ...
        if (!m_brdfValueCheckBox->checked() || (warpType != MicrofacetBRDF && warpType != PhongBRDF && warpType != BlinnBRDF))
            value_scale = 0.f;
        ...
        m_parameterSlider->setEnabled(warpType == CosinePowerHemisphere || warpType == Beckmann || warpType == MicrofacetBRDF || warpType == PhongBRDF || warpType == BlinnBRDF || warpType == UniformSphereCap);
        m_parameterBox->setEnabled(warpType == CosinePowerHemisphere || warpType == Beckmann || warpType == MicrofacetBRDF || warpType == PhongBRDF || warpType == BlinnBRDF || warpType == UniformSphereCap);
        m_angleBox->setEnabled(warpType == MicrofacetBRDF || warpType == PhongBRDF || warpType == BlinnBRDF);
        m_angleSlider->setEnabled(warpType == MicrofacetBRDF || warpType == PhongBRDF || warpType == BlinnBRDF);
        m_parameterBox->setEnabled(warpType == MicrofacetBRDF || warpType == PhongBRDF || warpType == BlinnBRDF);
        m_brdfValueCheckBox->setEnabled(warpType == MicrofacetBRDF || warpType == PhongBRDF || warpType == BlinnBRDF);
    ...
    void drawContents() {
        ...
        if (m_drawHistogram) {
            ...
            const int histHeight = (warpType == Square || warpType == Disk || warpType == Triangle) ? histWidth : histWidth / 2;
            ...
            if (m_warpTypeBox->selectedIndex() == MicrofacetBRDF || m_warpTypeBox->selectedIndex() == PhongBRDF || m_warpTypeBox->selectedIndex() == BlinnBRDF) {
                m_arrowShader->bind();
    ...
    void runTest() {
        ...
        if (warpType != Square && warpType != Disk && warpType != Triangle)
            xres *= 2;
        ...
        auto integrand = [&](double y, double x) -> double {
            if (warpType == Square) {
                ...
                else if (warpType == CosinePowerHemisphere)
                    return Warp::squareToCosinePowerHemispherePdf(v, parameterValue);
                else if (warpType == Beckmann)
                    return Warp::squareToBeckmannPdf(v, parameterValue);
                else if (warpType == MicrofacetBRDF || warpType == PhongBRDF || warpType == BlinnBRDF) {
                    BSDFQueryRecord bRec(m_bRec);
                    bRec.wo = v;
    ...
    void initializeGUI() {
        ...
        m_warpTypeBox = new ComboBox(m_window, { "Square", "Disk", "Triangle", "Sphere", "Spherical cap", "Hemisphere (unif.)",
                "Hemisphere (cos)", "Hemisphere (cos^n)", "Beckmann distr.", "Microfacet BRDF", "Phong BRDF", "Blinn BRDF" });
