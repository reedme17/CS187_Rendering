...
public:
    ...
    /// Return whether or not the scene has an environment emitter
    bool hasEnvEmitter() const { return m_envEmitter != nullptr; }

    /// Return a pointer to the environment emitter (if any)
    const Emitter *getEnvEmitter() const { return m_envEmitter; }

	/// Return whether or not the scene has a medium
	bool hasMedium() const { return m_medium != nullptr; }

	/// Return a pointer to the medium (if any)
    ...
    /// Uniformly pick a emitter and invoke its direct illumination sampling method
    Color3f sampleDirect(EmitterQueryRecord &lRec, const Point2f &sample) const;

    /// Compute the density of \ref sampleDirect()
    float pdfDirect(const EmitterQueryRecord &lRec) const;
